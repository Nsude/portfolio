...
const PostersGrid = () => {
  ...
  return (
    <div ref={containerRef} className="lg:h-[100dvh] w-full pb-[150px] md:pb-0">
      <div ref={gridCon} className="grid grid-cols-2 grid-rows-3 lg:grid-cols-8 gap-[5px] h-full w-full">
        {
          posters.map((image, i) => (
            <div ref={(el) => addElem(el, imagesRef.current)} key={i} className={`w-full h-full ${( device.width < 768 && i > 5 )? 'hidden' : 'inline-block'}`}>
